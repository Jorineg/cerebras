...
while IFS= read -r line || [ -n "$line" ]; do
    ...
    # Parse the 2-tuple: width,height
    IFS=',' read -r width height <<< "$line"
    
    # Trim whitespace
    width=$(echo "$width" | xargs)
    height=$(echo "$height" | xargs)
    
    echo "Testing: Grid ${width}x${height}"
    
    # Run benchmark
    benchmark_result=$(./benchmark_single.sh "$width" "$height")
    
    # Parse results
    IFS=' ' read -r wse2_cycles wse3_cycles <<< "$benchmark_result"
    
    # Store results
    results+=("$width|$height|$wse2_cycles|$wse3_cycles")
...
echo "| Grid Size | WSE2 Cycles/Iter | WSE3 Cycles/Iter |" >> experiment_results.md
echo "|-----------|------------------|------------------|" >> experiment_results.md

# Add results to table
for result in "${results[@]}"; do
    IFS='|' read -r width height wse2_cycles wse3_cycles <<< "$result"
    ...
    echo "| ${width}x${height} | $wse2_display | $wse3_display |" >> experiment_results.md
done
